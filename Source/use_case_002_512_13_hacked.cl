...
if(idGrid[centreIdx] == 1) {
		t1x0y0 = ((plateCoeffOne*modelGrid[t0x0y0Idx])
        + (plateCoeffTwo*(modelGrid[t0x1y0Idx]+modelGrid[t0xM1y0Idx]+modelGrid[t0x0y1Idx]+modelGrid[t0x0yM1Idx]))
        - (plateCoeffThree*(modelGrid[t0x1y1Idx]+modelGrid[t0xM1yM1Idx]+modelGrid[t0x1yM1Idx]+modelGrid[t0xM1y1Idx]))
        - (plateCoeffFour*(modelGrid[t0x2y0Idx]+modelGrid[t0xM2y0Idx]+modelGrid[t0x0y2Idx]+modelGrid[t0x0yM2Idx]))
        + (plateCoeffFive*modelGrid[tM1x0y0Idx])
        - (plateCoeffSix*(modelGrid[tM1x1y0Idx]+modelGrid[tM1xM1y0Idx]+modelGrid[tM1x0y1Idx]+modelGrid[tM1x0yM1Idx]))
        + excitation)
        / damp;
}
if(idGrid[centreIdx] == 2) {
		t1x0y0 = ((strOneCoeffOne*modelGrid[t0x0y0Idx])
        +(strOneCoeffTwo*(modelGrid[t0x1y0Idx]+modelGrid[t0xM1y0Idx]))
        -(strOneCoeffThree*(modelGrid[t0x2y0Idx]+modelGrid[t0xM2y0Idx]))
        +(strOneCoeffFour*modelGrid[tM1x0y0Idx])
        -(strOneCoeffFive*(modelGrid[tM1x1y0Idx]+modelGrid[tM1xM1y0Idx]))
        + excitation) / strDamp;
}
...
if(idGrid[centreIdx] == 4) {
		t1x0y0 = ((strOneCoeffOne*modelGrid[t0x0y0Idx])+(strOneCoeffTwo*(modelGrid[t0x1y0Idx]+modelGrid[t0xM1y0Idx]))-
        (strOneCoeffThree*(modelGrid[t0x2y0Idx]+modelGrid[t0xM2y0Idx]))+(strOneCoeffFour*modelGrid[tM1x0y0Idx])-(strOneCoeffFive*(modelGrid[tM1x1y0Idx]+
        modelGrid[tM1xM1y0Idx]))
        + excitation) / strDamp;
}
...
void connectionsKernel(__global int* idGrid, __global float* modelGrid, __global float* boundaryGrid, int idxRotate, int numConnections, __global int* connections, float plateDeltaH, float strDeltaH, float deltaT)
{
    ...
    if(connections[centreIdx] > 0)
    {
        ...
        // modelGrid[rotation1 + connections[centreIdx]] -= intermediateString;
            ...
            float intermediateOne = (modelGrid[t1x0y0Idx] - modelGrid[rotation1 + connections[centreIdx]]) / 100.0;

            modelGrid[t1x0y0Idx] -= intermediateOne;
            modelGrid[rotation1 + connections[centreIdx]] += intermediateOne;
    }
